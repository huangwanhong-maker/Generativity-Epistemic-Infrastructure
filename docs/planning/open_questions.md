...
| Q-44 | OPEN | Which access policies, CI checks and release procedures will serve the four source repositories? The user supplied all four GitHub URLs, and the submodule URLs now identify those hosted repositories; the current [repository guide](../development/git_repository_layers.md) records their use. Release and repository-governance policies remain open. MIT repository licensing does not license private application records or adopt governance drafts. | Repository governance and releases |
