...
def main() -> int:
    ...
    for doc, source, ids in source_entries:
        ...
        # Publication structure is an artifact check, not an editorial approval.
        headings = re.findall(r"\\section\{([^}]+)\}", source)
        if headings[:3] != ["Scope", "Normative references", "Terms and definitions"]:
            findings.append("Opening clauses do not match the declared formal structure")
        for marker in (r"\GSPfrontmatter", r"\GSPpreface{Foreword}",
                       r"\GSPpreface{Introduction}", r"\GSPmainmatter", r"\GSPbibliography"):
            if marker not in source:
                findings.append("Missing publication element: " + marker)
        if not re.search(r"\\GSPtitle\{[^\n]+\}\{" + re.escape(manifest["version"]) + r"\}", source):
            findings.append("Title version differs from manifest")
        scope = re.search(r"\\section\{Scope\}(.*?)\\section\{", source, re.S)
        if scope and r"\req{" in scope.group(1):
            findings.append("Requirement is located in the Scope clause")
        for status, body in re.findall(
                r"\\GSPannex\{([^}]+)\}\{[^\n]+\}(.*?)(?=\\GSPannex|\\GSPbibliography|\\end\{document\})",
                source, re.S):
            if status not in {"normative", "informative"}:
                findings.append("Unknown annex status: " + status)
            if status == "informative" and r"\req{" in body:
                findings.append("Informative annex contains a requirement declaration")
        labels = re.findall(r"\\label\{([^}]+)\}", source)
        duplicate_labels = [key for key, count in Counter(labels).items() if count > 1]
        if duplicate_labels:
            findings.append("Duplicate local labels: " + ", ".join(duplicate_labels))
        targets = set(labels) | {"req:" + identifier for identifier in ids}
        unresolved = set(re.findall(r"\\(?:ref|pageref)\{([^}]+)\}", source)) - targets
        if unresolved:
            findings.append("Unresolved local cross-references: " + ", ".join(sorted(unresolved)))
        for table in re.findall(r"\\begin\{longtable\}(.*?)\\end\{longtable\}", source, re.S):
            if r"\caption{" not in table:
                findings.append("Long table lacks a caption")
        ...
        rows.append({"id": name, "requirements": len(ids), "pages": pages,
                     "source_words": len(source.split()), "findings": findings})
    for archive in sorted((ROOT / "publications/archive").glob("*/snapshot.json")):
        snapshot = json.loads(archive.read_text(encoding="utf-8"))
        for archived in snapshot["files"]:
            path = ROOT / archived["archived"]
            if not path.is_file() or digest(path) != archived["sha256"]:
                errors.append("Historical snapshot changed: " + archived["archived"])
        for doc, source, ids in source_entries:
            old_path = archive.parent / doc["source"]
            if not old_path.is_file():
                continue
            old_ids = set(re.findall(r"\\req\{([A-Z][A-Z0-9]*-\d{3})\}", old_path.read_text(encoding="utf-8")))
            if old_ids - set(ids):
                errors.append(doc["id"] + ": historical requirement IDs removed: " + ", ".join(sorted(old_ids - set(ids))))
    result = {
        "checked_at_utc": datetime.now(timezone.utc).isoformat(),
        "version": manifest["version"],
        "scope": "Source metadata, formal structure, unique/resolved requirement IDs and local references, archive hashes, retained historical IDs, full-build freshness, PDF hashes and extracted requirement coverage. Not substantive conformance, rendering completeness, or accessibility assurance.",
